    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <div class="flex items-center gap-3 px-5 py-5 border-b border-white/10">
            ...
            <div>
                ...
                <p class="text-blue-300 text-xs">{{ Auth::user()->hasRole('Admin') ? 'Admin' : 'Manager' }}</p>
            </div>
        </div>
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
            ...
            <a href="{{ route('manager.activity.index') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors
                      {{ request()->routeIs('manager.activity.*') ? 'bg-white/15 text-white' : 'text-blue-100 hover:bg-white/10 hover:text-white' }}">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
        ...
        <header class="shrink-0 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 px-4 sm:px-6 h-14 flex items-center justify-between shadow-sm transition-colors duration-200">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-gray-400 dark:text-gray-500 text-xs hidden sm:block">@yield('page-subtitle')</p>
                    @endif
                </div>
            </div>
            <div class="flex items-center gap-2">
                ...
                <span class="hidden sm:inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium
                             {{ Auth::user()->hasRole('Admin') ? 'bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border border-purple-100 dark:border-purple-800' : 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border border-blue-100 dark:border-blue-800' }}">
