...
@section('content')
    ...
    <div class="flex items-center gap-2 text-xs text-gray-400 dark:text-gray-500 mb-5">
        <a href="{{ route('employee.dashboard') }}"
            class="hover:text-gray-600 dark:hover:text-gray-300 transition-colors">Dashboard</a>
        <span>/</span>
        <a href="{{ route('employee.tasks.index') }}"
            class="hover:text-gray-600 dark:hover:text-gray-300 transition-colors">Tugas Saya</a>
        <span>/</span>
    ...
    @php
        ...
        $existing = $assignment->activities
            ->map(fn($a) => ['description' => $a->description, 'status' => $a->status])
            ->values();
        if ($existing->isEmpty()) {
            $existing = collect([['description' => '', 'status' => 'done']]);
        }
    @endphp
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-5">
        ...
        <div class="lg:col-span-3 space-y-4">
            ...
            {{-- Header --}}
            <div
                class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 sm:p-6 transition-colors duration-200">
                <div class="flex flex-wrap gap-2 mb-3">
                    <x-status-badge :status="$progress" />
                    <x-priority-badge :priority="$task?->priority ?? 'medium'" />
                    @if ($isOverdue)
                        <span
                            class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 border border-red-100 dark:border-red-800">Terlambat</span>
                    @endif
                </div>
                ...
                <dl
                    class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm mt-5 pt-4 border-t border-gray-100 dark:border-gray-700">
                    <div>
                    ...
                    <div>
                        ...
                        <dd
                            class="font-medium {{ $isOverdue ? 'text-red-600 dark:text-red-400' : 'text-gray-800 dark:text-gray-200' }}">
                    ...
                    <div>
                        ...
                        <dd class="font-medium text-gray-800 dark:text-gray-200">
                            {{ $assignment->started_at?->translatedFormat('d M Y, H:i') ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-gray-400 dark:text-gray-500 text-xs mb-0.5">Durasi pengerjaan</dt>
                        <dd class="font-medium text-gray-800 dark:text-gray-200">
                            {{ $assignment->workDurationHuman() ?? '—' }}</dd>
                    </div>
                </dl>
            </div>
            ...
            @if ($assignment->manager_notes && in_array($progress, ['revision', 'done']))
                <div
                    class="rounded-2xl border p-5 {{ $progress === 'revision' ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800' : 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' }}">
                    <p
                        class="text-sm font-semibold {{ $progress === 'revision' ? 'text-amber-800 dark:text-amber-300' : 'text-green-800 dark:text-green-300' }} mb-1">
                        {{ $progress === 'revision' ? 'Catatan Revisi dari Manager' : 'Catatan Manager' }}
                    </p>
                    <p
                        class="text-sm {{ $progress === 'revision' ? 'text-amber-700 dark:text-amber-400' : 'text-green-700 dark:text-green-400' }} leading-relaxed">
                        {{ $assignment->manager_notes }}</p>
                </div>
            @endif
            ...
            @if (in_array($progress, ['submitted', 'done']) && $assignment->activities->isNotEmpty())
                <div
                    class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 transition-colors duration-200">
                    ...
                    <ul class="space-y-2">
                        @foreach ($assignment->activities as $act)
                            <li class="flex items-start gap-2.5">
                                @if ($act->status === 'done')
                                    <svg class="w-4 h-4 text-green-500 mt-0.5 shrink-0" fill="currentColor"
                                        viewBox="0 0 20 20">
                                ...
                                @else
                                    <svg class="w-4 h-4 text-amber-500 mt-0.5 shrink-0" fill="currentColor"
                                        viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                                ...
                                <span class="text-sm text-gray-700 dark:text-gray-300">{{ $act->description }}
                                    @if ($act->status === 'blocked')
                                        <span class="text-xs text-amber-600 dark:text-amber-400">(terkendala)</span>
                                    @endif
                                </span>
                            </li>
                        @endforeach
                    ...
                    @if ($assignment->attachments->isNotEmpty())
                        <div class="mt-4 pt-3 border-t border-gray-100 dark:border-gray-700">
                            <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">Lampiran
                                ({{ $assignment->attachments->count() }})</p>
                            <div class="flex flex-wrap gap-2">
                                @foreach ($assignment->attachments as $att)
                                    <a href="{{ $att->url() }}" target="_blank"
                                        class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                        @if ($att->isImage())
                                        ...
                                        <span class="text-gray-400 dark:text-gray-500">{{ $att->humanSize() }}</span>
                                    </a>
                                @endforeach
                            </div>
                        </div>
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div
                ...
                @if ($progress === 'not_started')
                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-4">Klik untuk mulai mengerjakan tugas. Waktu mulai
                        akan dicatat.</p>
                    <form method="POST" action="{{ route('employee.tasks.progress', $assignment->id) }}">
                        @csrf @method('PATCH')
                        <input type="hidden" name="progress" value="on_progress">
                        <button type="submit"
                            class="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-semibold text-white rounded-xl"
                ...
                @elseif ($canEdit)
                    ...
                    @if ($progress === 'revision')
                        <div
                            class="flex items-center gap-2 p-3 rounded-xl bg-amber-50 dark:bg-amber-900/30 border border-amber-100 dark:border-amber-800 mb-4">
                            <svg class="w-4 h-4 text-amber-600 dark:text-amber-400 shrink-0" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                            </svg>
                            <span class="text-xs font-medium text-amber-700 dark:text-amber-300">Perlu revisi — perbaiki &
                                kirim ulang</span>
                        </div>
                    @endif

                    <form method="POST" action="{{ route('employee.tasks.submit', $assignment->id) }}"
                        enctype="multipart/form-data" x-data="{ items: {{ Illuminate\Support\Js::from($existing) }} }">
                        @csrf
                        ...
                        <div class="space-y-2 mb-2">
                            <template x-for="(item, idx) in items" :key="idx">
                                <div class="flex items-start gap-2">
                                    <div class="flex-1">
                                        <input type="text" :name="'activities[' + idx + '][description]'"
                                            x-model="item.description" placeholder="Aktivitas yang dikerjakan..."
                        ...
                        @error('activities')
                            <p class="text-xs text-red-600 mb-3 -mt-2">{{ $message }}</p>
                        @enderror
                        ...
                        <textarea name="communication_note" rows="3" placeholder="Masukan catatan disini..."
                        ...
                        <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                            Lampiran Bukti
                            <span class="font-normal text-gray-400 dark:text-gray-500">(opsional — JPG/PNG/PDF, maks 5 file
                                @ 5 MB)</span>
                        </label>
                        <input type="file" name="attachments[]" multiple accept=".jpg,.jpeg,.png,.pdf"
                            class="block w-full text-xs text-gray-600 dark:text-gray-300 mb-4
                                  file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium
                        ...
                        @if ($assignment->attachments->isNotEmpty())
                            <p class="text-[11px] text-gray-400 dark:text-gray-500 mb-3">
                                {{ $assignment->attachments->count() }} lampiran sudah terunggah — file baru akan
                                ditambahkan.
                            </p>
                        @endif
                ...
                @elseif ($progress === 'submitted')
                    <div class="flex flex-col items-center py-4 text-center">
                        <div
                            class="w-12 h-12 rounded-full bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center mb-3">
                        ...
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Laporan dikirim
                            {{ $assignment->submitted_at?->translatedFormat('d M Y, H:i') }}</p>
                    </div>
                @elseif ($progress === 'done')
                    <div class="flex flex-col items-center py-4 text-center">
                        <div
                            class="w-12 h-12 rounded-full bg-green-50 dark:bg-green-900/30 flex items-center justify-center mb-3">
                        ...
                        @if ($assignment->kpi_score !== null)
                            <p class="mt-2"><span
                                    class="inline-flex items-center px-2.5 py-1 rounded-lg text-sm font-bold
